  ...
  try {
    ...
    if (!document.getElementById("vcms-theme-ext-css")) {
      var css = "@media screen{" +
        ...
        "html[data-vcms-theme=on] body{font-family:var(--vcms-font);font-size:var(--vcms-fs-base,14px)}" +
        "html[data-vcms-theme=on] h1,html[data-vcms-theme=on] h2,html[data-vcms-theme=on] h3{color:var(--vcms-heading);font-weight:var(--vcms-fw-h,800)}" +
        "html[data-vcms-theme=on] button,html[data-vcms-theme=on] .vcms-btn{font-weight:var(--vcms-fw-btn,800);transition-duration:var(--vcms-tspeed,220ms)}" +
    ...
    function applyBundle(b) {
      if (!b || !b.themes || !b.active) return false;
      var t = b.themes[b.active]; if (!t || !t.colors) return false;
      var C = t.colors, S = t.shapes || {}, T = t.type || {}, A = t.anim || {};
      var rs = document.documentElement.style;
      function s(k, v) { if (v !== undefined && v !== null) rs.setProperty(k, v); }
      ...
      s("--vcms-shadow-alpha", Math.max(0, Math.min(1.4, Number(S.shadow) || 0)));
      ...
      if (F) s("--vcms-font", F);
      s("--vcms-fs-base", ci(T.fsBase, 12, 17) + "px");
      s("--vcms-fs-h", ci(T.fsH, 16, 26) + "px");
      s("--vcms-fw-h", ci(T.fwH, 600, 800));
      s("--vcms-fw-btn", ci(T.fwBtn, 600, 800));
    ...
    window.VCMS_THEME_EXT = { apply: applyBundle };
    window.addEventListener("vcmscompanytheme", function (event) { applyBundle(event.detail); });
    var b = null;
