  ...
  window.addEventListener('load',function(){navigator.serviceWorker.register('sw.js').then(function(reg){
    ...
    reg.addEventListener('updatefound',function(){var nw=reg.installing;if(!nw)return;nw.addEventListener('statechange',function(){
      if(nw.state==='installed'&&navigator.serviceWorker.controller&&!document.getElementById('vmms-update')){
        var b=document.createElement('button');b.id='vmms-update';b.textContent='VCMS update ready — tap to refresh';b.style.cssText='position:fixed;left:12px;right:12px;bottom:calc(14px + env(safe-area-inset-bottom,0px));z-index:10001;border:0;border-radius:11px;background:#202631;color:#fff;padding:12px;font:800 13px system-ui;box-shadow:0 8px 24px rgba(0,0,0,.25)';b.onclick=function(){location.reload()};document.body.appendChild(b);
      }
    })});
  }).catch(function(){})});
})();
