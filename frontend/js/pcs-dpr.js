...
(function(){
  ...
  async function loadBlocks(){
    ...
    updatePcsReadiness();
    // Render the location forms immediately on slow 4G. Manpower is a
    // secondary roster and fills in as soon as that request completes.
    vmmsApi("/api/v1/pcs/distribution?site_id="+encodeURIComponent(SITE)+"&distribution_date="+encodeURIComponent(DATE)).then(function(dr){if(!dr.ok)return null;return dr.json()}).then(function(data){if(!data)return;DIST=data;BLOCKS.forEach(function(b){if(b.refreshManpower)b.refreshManpower()})}).catch(function(){});
  }
  ...
  function buildBlock(loc,saved){
    ...
    box.innerHTML='<button type="button" class="pcs-loc-head" data-toggle><span class="pcs-chevron">⌄</span><h3>'+esc(loc.name)+'</h3><span class="pcs-status '+(submitted?'submitted':'draft')+'" data-st>'+(submitted?'submitted':'draft')+'</span></button><div class="pcs-loc-body"><div class="sub">'+esc(loc.code||"")+' · Reported by: '+esc(reporter)+'</div><div class="pcs-sync" data-sync>'+(saved&&saved.updated_at?'Last synced '+esc(new Date(saved.updated_at).toLocaleString('en-SG')):'Saved automatically on this device')+'</div>'+
      ...
      '<div class="pcs-sech">Today activities (cumulative % as of today)</div><div class="pcs-col-head today"><span>Activity description</span><span>Previous %</span><span>Current %</span><span>Status</span><span>Remarks</span><span></span></div><div data-today></div><button class="pcs-btn sm" data-addtoday>+ Activity</button>'+
      '<div class="pcs-sech">Tomorrow activities</div><div class="pcs-col-head tomorrow"><span>Activity description</span><span>Remarks</span><span></span></div><div data-tom></div><button class="pcs-btn sm" data-addtom>+ Activity</button>'+
    ...
    function renderList(host,arr,cols,kind){
      ...
      arr.forEach(function(item,idx){
        var row=ce("div","pcs-row"+(kind?" activity-row "+kind+"-row":""));
