...
def build_schedule_router(context: ScheduleModuleContext) -> APIRouter:
    ...
    @router.get("/progress")
    async def list_progress(project_id: str, activity_id: str | None = None, user: dict = Depends(context.get_current_user)):
        async with context.shared_client() as client: return await progress_service(client,user).list(project_id,activity_id)
